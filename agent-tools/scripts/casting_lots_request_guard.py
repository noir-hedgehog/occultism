#!/usr/bin/env python3
"""Guard bone, shell, stone, and charm-casting requests before consultation."""
...
INTENT_KEYWORDS = {
    "casting_lots_symbolic_consultation": ("骨占", "贝壳占卜", "石子占卜", "符物占卜", "小物占卜", "小物抛掷", "符物抛掷", "抛掷占卜", "撒骨", "casting lots", "charm casting", "bone casting", "shell divination"),
    "casting_record": ("抛到", "撒到", "落在", "小物", "骨片", "贝壳", "石子", "符物", "casting mat", "casting cloth"),
    "cultural_learning": ("文化", "讲讲", "是什么意思", "学习", "来历", "传统"),
}

RISK_KEYWORDS = {
    "professional_replacement": ("不用医生", "不用律师", "不用报警", "不用吃药", "只看骨", "只看贝壳", "全靠小物"),
    ...
    "animal_or_human_remains": ("人骨", "尸骨", "骨灰", "动物尸体", "新鲜骨头", "血", "献祭", "杀动物"),
    "repeated_dependency": ("反复抛", "抛到满意", "一直抛", "停不下来", "不抛不敢决定"),
}
...
def detect_intent(text: str) -> str:
    for intent, keywords in INTENT_KEYWORDS.items():
        if contains_any(text, keywords):
            return intent
    return "casting_lots_symbolic_reflection"
...
def reframe(flags: list[str]) -> str:
    if "animal_or_human_remains" in flags:
        return "不处理人类遗骸、动物伤害、血祭或违法/不安全材料；可改用纸片、石子、贝壳复制品等无害象征物。"
    ...
    if "professional_replacement" in flags or "medical_or_safety" in flags:
        return "符物抛掷不能替代医疗、法律、安全或心理健康支持；先处理现实专业支持，再把盘面作为象征提醒。"
    if "financial_or_gambling" in flags:
        return "不使用抛掷结果决定投资、贷款、彩票、赌博或高风险财务行为；可改成风险清单和现实约束。"
    if "deterministic_fate" in flags:
        return "不把抛掷盘面写成确定预言或命运证明；可改成可能性、现实证据和低风险下一步。"
    if "third_party_privacy" in flags or "coercion_or_control" in flags:
        return "不使用符物抛掷窥探、操控或强迫他人；可改成自己的边界、沟通和可控行动。"
    if "repeated_dependency" in flags:
        return "暂停反复抛掷直到满意的依赖模式；先固定问题、次数和现实验证步骤。"
    return "可以把骨、贝壳、石子或符物抛掷作为象征性反思工具，不作为事实证明、专业建议或结果保证。"
...
def guard(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "tool": "casting_lots_request_guard",
        "request_text": text,
        "system": "casting_lots_symbolic_reflection",
        "consultation_intent": detect_intent(text),
        "risk_flags": flags,
        "can_continue_casting_lots": allowed,
        "reframed_question": reframe(flags),
        "required_boundaries": [
            "骨、贝壳、石子和符物抛掷只作为象征性反思、问题整理和低风险行动提醒，不证明事实、不保证结果。",
            "不替代医疗、法律、财务、安全、心理健康或紧急支持。",
            "不用于遗骸、动物伤害、血祭、赌博投资、第三方窥探、操控、确定预言、驱邪证明或反复依赖。",
        ],
        "clarifying_questions": [
            "用户是想文化学习、记录已有盘面，还是做低风险象征咨询？",
            "使用的是哪类符物、投掷垫/区域和读取规则；盘面是否已经固定？",
            "请求是否涉及专业替代、财务赌博、灵异恐惧、遗骸/伤害、第三方隐私、操控或反复依赖？",
        ],
        "next_steps": [
            "record_casting_lots_layout",
            "lookup_casting_lots_symbols",
            "build_casting_lots_interpretation_plan",
            "lint_final_output_with_mystic_output_lint",
        ] if allowed else ["pause_casting_lots_consultation", "reframe_to_real_world_support"],
    }
...
def main() -> int:
    ...
    parser.add_argument("--text", help="Bone, shell, stone, or charm-casting request text.")
    ...
    return 0 if result["can_continue_casting_lots"] else 1
