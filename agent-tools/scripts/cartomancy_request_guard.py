#!/usr/bin/env python3
"""Guard playing-card cartomancy requests before symbolic consultation."""
...
INTENT_KEYWORDS = {
    "cartomancy_symbolic_consultation": ("扑克牌占卜", "扑克牌算命", "纸牌占卜", "扑克牌解读", "cartomancy", "playing card reading"),
    "card_record": ("红桃", "黑桃", "方片", "梅花", "spades", "hearts", "diamonds", "clubs", "joker", "扑克牌"),
    "cultural_learning": ("文化", "讲讲", "是什么意思", "学习", "来历", "传统"),
}

RISK_KEYWORDS = {
    "professional_replacement": ("不用医生", "不用律师", "不用报警", "不用吃药", "只看牌", "全靠扑克牌"),
    ...
    "repeated_dependency": ("反复抽", "抽到满意", "一直抽", "停不下来", "不抽不敢决定"),
}
...
def detect_intent(text: str) -> str:
    for intent, keywords in INTENT_KEYWORDS.items():
        if contains_any(text, keywords):
            return intent
    return "cartomancy_symbolic_reflection"
...
def reframe(flags: list[str]) -> str:
    if "professional_replacement" in flags or "medical_or_safety" in flags:
        return "扑克牌占卜不能替代医疗、法律、安全或心理健康支持；先处理现实专业支持，再把牌面作为象征提醒。"
    if "financial_or_gambling" in flags:
        return "不使用扑克牌决定投资、贷款、彩票、赌博或高风险财务行为；可改成风险清单和现实约束。"
    if "deterministic_fate" in flags:
        return "不把牌面写成确定预言或命运证明；可改成可能性、现实证据和低风险下一步。"
    if "third_party_privacy" in flags or "coercion_or_control" in flags:
        return "不使用扑克牌窥探、操控或强迫他人；可改成自己的边界、沟通和可控行动。"
    if "repeated_dependency" in flags:
        return "暂停反复抽牌直到满意的依赖模式；先固定问题、牌阵和现实验证步骤。"
    return "可以把扑克牌作为象征性反思工具，不作为事实证明、专业建议或结果保证。"
...
def main() -> int:
    ...
    parser.add_argument("--text", help="Playing-card cartomancy request text.")
    ...
    return 0 if result["can_continue_cartomancy"] else 1
